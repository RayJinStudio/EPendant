...
int AppController::main_process(UserAction *act_info)
{

    if (ACTIVE_TYPE::UNKNOWN == act_info->active) return 0;
    if (ACTIVE_TYPE::UNKNOWN != act_info->active)
    {
        printf("[Operate]\tact_info->active: %d\n",act_info->active);
    }
    // if (isRunEventDeal)
    // {
    ...
    {
        if(ACTIVE_TYPE::RETURN == act_info->active)
        {
            app_exit();
            delay(500);
        }
...
void AppController::app_exit()
{
    ...
    //     setCpuFrequencyMhz(80);
    // }
    // printf("CpuFrequencyMhz: ");
    // printf(getCpuFrequencyMhz());
}